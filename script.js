...
class GroovyFish {
  constructor() {
    ...
    this.isMobile = false;
    this.mobileScale = 1.0; // 1.0 = desktop, <1.0 = mobile (makes everything smaller)
  ...
  resizeCanvas() {
    ...
    // Scale factor: on mobile everything draws in a larger internal space
    // so when CSS shrinks it to fit, all entities appear smaller
    this.mobileScale = isMobile ? 0.55 : 1.0;
    const invScale = 1 / this.mobileScale;

    if (isMobile && isPortrait) {
      // Portrait mobile: rotate canvas to landscape
      this.isRotated = true;
      const cssW = vh; // visual landscape width = screen height
      const cssH = vw; // visual landscape height = screen width
      this.canvas.width = Math.round(cssW * invScale);  // larger internal resolution
      this.canvas.height = Math.round(cssH * invScale);
      ...
      // Landscape mobile
      this.isRotated = false;
      this.canvas.width = Math.round(vw * invScale);
      this.canvas.height = Math.round(vh * invScale);
      this.canvas.style.transform = "none";
      this.canvas.style.transformOrigin = "";
      this.canvas.style.width = vw + "px";
      this.canvas.style.height = vh + "px";
    } else {
      // Desktop: normal (no scaling)
      this.isRotated = false;
  ...
  // Remap screen coordinates to canvas coordinates (accounting for rotation + scale)
  remapCoords(screenX, screenY) {
    const invScale = 1 / this.mobileScale;
    if (this.isRotated) {
      // When rotated 90° CW and scaled:
      // screen coords → rotated → scaled to internal canvas coords
      return {
        x: screenY * invScale,
        y: (window.innerWidth - screenX) * invScale,
      };
    }
    // Landscape mobile or desktop
    return { x: screenX * invScale, y: screenY * invScale };
  }
  ...
  createLilyPads() {
    this.lilyPads = [];
    const count =
      Math.floor((this.canvas.width * this.canvas.height) / 80000) + 4;
  ...
  createInitialFish() {
    ...
    for (let i = 0; i < 2; i++) {
      this.fish.push(
        new AsianCarp(this, fishTypes[(i + 3) % fishTypes.length]),
      );
    }
  }
  ...
  createFrogs() {
    this.frogs = [];
    for (let i = 0; i < 3; i++) this.frogs.push(new Frog(this));
  }
  createFlies() {
    this.flies = [];
    for (let i = 0; i < 4; i++) this.flies.push(new Fly(this));
  }
  createBirds() {
    this.birds = [];
    for (let i = 0; i < 2; i++) this.birds.push(new Bird(this));
  }
  createTrunks() {
    this.trunks = [];
    this.trunks.push(new Trunk(this));
  }
  createTurtles() {
    this.turtles = [];
    for (let i = 0; i < 4; i++) this.turtles.push(new Turtle(this));
  }
  createCormorants() {
    this.cormorants = [];
    for (let i = 0; i < 3; i++) this.cormorants.push(new Cormorant(this));
  }
  ...
  drawCaustics(ctx) {
    ...
    for (let x = 0; x < this.canvas.width; x += step) {
      for (let y = 0; y < this.canvas.height; y += step) {
        const noise =
          Math.sin(x * 0.01 + this.causticPhase) *
          Math.cos(y * 0.01 + this.causticPhase * 1.3);
        const r = 20 + noise * 15;
        ctx.fillStyle = "#FFFFFF";
        ctx.beginPath();
        ctx.arc(
          x + noise * 10,
          y + Math.cos(x * 0.02 + this.causticPhase) * 8,
          r,
          0,
          Math.PI * 2,
        );
